...
{
    public class RepositoryUriSupportResult
    {
        public RepositoryUriSupportResult(bool isSupported)
        {
            IsSupported = isSupported;
            Result = new ProviderOperationResult(ProviderOperationStatus.Success, null, string.Empty, string.Empty);
        }

        public RepositoryUriSupportResult(Exception error, string diagnosticText)
        {
            IsSupported = false;
